...
class RDT3_0:
    ...
    def sendFrame(self, frame: int) -> int:
        if self.timeout:
            self.timeout = False
            print(f"Sender: Timeout. Resending frame {frame} [SEQ {frame % 2}].")
        elif self.isResponseCorrupted():
    ...
    def receiveFrame(self, frame: int) -> bool:
        if self.isLost():
            print(f"Receiver: Frame {frame} [SEQ {frame % 2}] lost.")
            time.sleep(random.uniform(1.5, 2.5))
            self.timeout = True
            return False
...
if __name__ == "__main__":
    ...
    print("Simulating RDT 3.0 Protocol:~\n")
    protocol = RDT3_0()
